...
const OrderSuccessPage = () => {
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      const autoTable = (await import("jspdf-autotable")).default; // Note: autotable might need default or just side-effect import
      ...
      try {
        doc.autoTable({
         ...
         console.warn("AutoTable error, trying fallback import", err);
         // Fallback if autoTable is not on doc directly (sometimes depends on version)
         if (autoTable) {
             autoTable(doc, {
                startY: yPos + 8,
                head: [["No", "Produk", "Qty", "Harga", "Subtotal"]],
      ...
      // Summary Box
      // Check if lastAutoTable exists, if not use estimation
      ...
      alert("Maaf, terjadi kesalahan saat mengunduh invoice. Silakan coba lagi.");
    }
  };
